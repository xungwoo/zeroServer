<@layout.extends name="/layout/base.ftl">
	...
	<@layout.put block="page-javascript-inline">
		...
		<script type="text/javascript">
			var serverUrl = {
				equipGrid : "${rc.contextPath}" + "${baseUrl}" + "/grid",
				equipGridEdit : "${rc.contextPath}" + "${baseUrl}" + "/grid/edit",
			}
			...
			function colorFomatter(cellvalue, options, rowObject) {
				var yFormat = "<span style='color:blue;font-weight:bold;'>"
				var nFormat = "<span style='color:red;font-weight:bold;'>"
				var closeFormat = "</span>";
				var result = "";
				if (cellvalue == 'Y') {
					result = yFormat + "Yes" + closeFormat;
				} else {
					result = nFormat + "No" + closeFormat;
				}
   				return result;
			}
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					url: serverUrl.equipGrid,
					mtype: "POST",
					datatype: "json",
					colNames:[' ', 'Key', '장비명', '장비Code', '상세분류', '메타정보유무'],					
					colModel:[
						...
						{name:'typeKey', index:'typeKey', sorttype:"integer", editable: true, key: true, hidden:true},
						{name:'typeName', index:'typeName', width:200, fixed:true, sorttype:"integer", editable: true },
						{name:'typeCode', index:'typeCode', width:120, fixed:true, sorttype:"integer", editable: true },
						{name:'subCategoryCode', index:'categoryCode', width:120, fixed:true, sorttype:"integer", editable: true, formatter:'select', edittype:"select", editoptions:{ value : "${equipSubCategoryTypeList}" }, searchoptions:{sopt: ['eq']} },
						{name:'hasMeta', index:'hasMeta', width:120, fixed:true, align:"center", sorttype:"integer", editable: false, formatter:colorFomatter},
					], 
					search : {
						caption: "Search...",
						Find: "Find",
					...
					editurl: serverUrl.equipGridEdit,
