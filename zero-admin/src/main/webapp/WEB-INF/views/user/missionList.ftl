	...
	<@layout.put block="page-javascript-inline">
		...
		<script type="text/javascript">
			$("#shardIndex").change(function() {
				location.href = "${rc.contextPath}/user/mission/grid/" + this.value;
			});
			...
			var serverUrl = {
				missionGrid : "${rc.contextPath}/user/mission/grid/${shardIndex}",
				missionGridEdit : "${rc.contextPath}/user/mission/grid/edit/${shardIndex}",
				missionExcel : "${rc.contextPath}/user/mission/excel/${shardIndex}",
				missionCsv : "${rc.contextPath}/user/mission/csv/${shardIndex}"
			}
			...
			jQuery(function($) {
				...
				var editOptions = {
				     ...
				     onclickSubmit: function(params){
				     	...
				     	ajaxData = { missionKey: rowData.missionKey };
				     	return ajaxData;
					},
				...
				jQuery(grid_selector).jqGrid({
					url: serverUrl.missionGrid,
					mtype: "POST",
					datatype: "json",
					colNames:[' ', 'Key', 'UserID', 'MissionID', 'Present Status', 'Receive the reward'],					
					colModel:[
						...
						{name:'missionKey', index:'missionKey', sorttype:"integer", editable: true, key: true, hidden:true, sortable:false},
						{name:'accountId', index:'accountId', width:300, fixed:true, sorttype:"string", editable: false, sortable:false, searchoptions:{sopt: ['eq']} },
						{name:'missionId', index:'missionId', width:120, fixed:true, sorttype:"integer", editable: false, sortable:false, search:false },
						{name:'current', index:'current', width:120, fixed:true, sorttype:"integer", editable: true, sortable:false, search:false },
						{name:'rewardDone', index:'rewardDone', width:120, fixed:true, sorttype:"integer", editable: true, search:false, edittype:"checkbox", formatter:"checkbox", sortable:false },
					], 
					search : {
						caption: "Search...",
						Find: "Find",
					...
					editurl: serverUrl.missionGridEdit,
				...
				onClickButton : function() {
					jQuery(grid_selector).jqGrid('excelExport', {url: serverUrl.missionExcel});
				}});
				
				jQuery(grid_selector).jqGrid('navButtonAdd',pager_selector,{caption:"CSV",title:"CSV Download",
				onClickButton : function() {
					jQuery(grid_selector).jqGrid('excelExport', {url: serverUrl.missionCsv});
				}});
