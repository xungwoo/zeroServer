	...
	<@layout.put block="page-javascript-inline">
		...
		<script type="text/javascript">
			...
			$("#shardIndex").change(function() {
				location.href = "${rc.contextPath}/user/deck/grid/" + this.value;
			});
			...
			var serverUrl = {
				deckGrid : "${rc.contextPath}/user/deck/grid/${shardIndex}",
				deckGridEdit : "${rc.contextPath}/user/deck/grid/edit/${shardIndex}",
				deckExcel : "${rc.contextPath}/user/deck/excel/${shardIndex}",
				deckCsv : "${rc.contextPath}/user/deck/csv/${shardIndex}"
			}
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					url: serverUrl.deckGrid,
					mtype: "POST",
					datatype: "json",
					colNames:['Key', '유저ID', '팀ID', '포지션', '유닛ID', 'Image', '유닛<br>Code', '유닛<br>Type', '유닛<br>Level', '스킬0레벨<br>(패시브)', '스킬1레벨<br>(액티브1)', '스킬2레벨<br>(액티브2)'],					
					colModel:[
						{name:'deckKey', index:'deckKey', sorttype:"integer", editable: false, key: true, hidden:true},
						{name:'accountId', index:'accountId', width:280, fixed:true, sorttype:"string", editable: false, sortable:false, searchoptions:{sopt: ['eq']} },
						{name:'teamId', index:'teamId', width:120, fixed:true, sorttype:"integer", editable: false, hidden:true, search:false, sortable:false },
						{name:'positionId', index:'positionId', width:60, fixed:true, sorttype:"integer", editable: false, search:false, sortable:false },
						{name:'unitId', index:'unitId', width:300, fixed:true, sorttype:"string", editable: false, sortable:false, searchoptions:{sopt: ['eq']} },
						...
						{name:'level', index:'level', width:60, fixed:true, sorttype:"integer", editable: false , sortable:false, search:false},
						{name:'skill0Lv', index:'skill0Lv', width:80, fixed:true, sorttype:"integer", editable: false , sortable:false, search:false},
						{name:'skill1Lv', index:'skill1Lv', width:80, fixed:true, sorttype:"integer", editable: false , sortable:false, search:false},
						{name:'skill2Lv', index:'skill2Lv', width:80, fixed:true, sorttype:"integer", editable: false , sortable:false, search:false},						
					], 
					search : {
						caption: "Search...",
						Find: "Find",
					...
					editurl: serverUrl.deckGridEdit,
					caption: "Deck Data",
				...
				onClickButton : function() {
					jQuery(grid_selector).jqGrid('excelExport', {url: serverUrl.deckExcel});
				}});
				
				jQuery(grid_selector).jqGrid('navButtonAdd',pager_selector,{caption:"CSV",title:"CSV Download",
				onClickButton : function() {
					jQuery(grid_selector).jqGrid('excelExport', {url: serverUrl.deckCsv});
				}});
