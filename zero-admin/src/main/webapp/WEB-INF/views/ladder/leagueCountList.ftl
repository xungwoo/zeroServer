	...
	<@layout.put block="page-javascript-inline">
		...
		<script type="text/javascript">
			$("#shardIndex").change(function() {
				location.href = "${rc.contextPath}/league/count/grid/" + this.value;
			});
			...
			var serverUrl = {
				ladderGrid : "${rc.contextPath}/league/count/grid/${shardIndex}",
				ladderGridEdit : "${rc.contextPath}/league/count/grid/edit/${shardIndex}",
				ladderExcel : "${rc.contextPath}/league/count/excel/${shardIndex}",
				ladderCsv : "${rc.contextPath}/league/count/csv/${shardIndex}"
			}
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					mtype: "POST",
					datatype: "json",
					colNames:[' ', 'Key', 'Image', '리그', '카운트', '리그종료일<br>(보상지급일)'],					
					colModel:[
						...
						{name:'countKey', index:'countKey', width:260, fixed:true, sorttype:"integer", hidden: true, key: true, editable: false, searchoptions:{sopt: ['eq']}},
						...
						{name:'league', index:'league', width:150, fixed:true, sorttype:"integer", formatter:"select", editable: true, edittype:"select", editoptions:{value:leagueOpts}, search:true, sortable:false, searchoptions: { sopt: ['eq'], value: ":All;"+leagueOpts }, stype:'select'},
						{name:'count', index:'count', width:80, fixed:true, sorttype:"integer", editable: true, searchoptions:{sopt: ['eq','ge','le']}, sortable:false},
						{name:'resetDate', index:'resetDate', width:100, fixed:true, sorttype:"integer", editable: true, sortable:false, search:true}
					], 
					search : {
						caption: "Search...",
						Find: "Find",
