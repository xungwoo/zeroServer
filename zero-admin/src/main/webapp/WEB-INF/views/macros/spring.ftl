 ...
 *    or CSS styles or size
 -->
<#macro formInput path attributes="" fieldType="text"><@compress single_line=true>
    <@bind path/>
...
<#macro formInputWithEscaping path attributes="" fieldType="text">
    <@bind path/>
 ...
 *    or CSS styles or size
 -->
<#macro formTextareaWithEsacping path attributes="">
    <@bind path/>
 ...
 *    or CSS styles or size
-->
<#macro formSingleSelect path options attributes="" optional=false totalName='전체'>
    <@bind path/>
    <select id="${status.expression}" name="${status.expression}" ${attributes}>
    	<#if optional!'false' >
    		<option value="" >${(totalName)?html}</option>
    	</#if>
        <#if options?is_hash>
 ...
 *    or CSS styles or size
-->
<#macro formCheckbox path attributes="" forceChecked=false><@bind path /><#assign id="${status.expression}"><#assign isSelected = status.value?? && status.value?string=="true"><#if forceChecked><#assign isSelected=true></#if><input type="hidden" name="_${id}" value="on"/><input type="checkbox" id="${id}" name="${id}"<#if isSelected> checked="checked"</#if> ${attributes}/></#macro>
 ...
 * errorMessage
 *
 * Macro to translate a error message code into a message.
 -->
<#macro errorMessage error>
    <#assign nomessage="___NOMESSAGE__">
    <#if error.arguments?? && (error.arguments?size > 1) >
        <#assign args = error.arguments[1..] />
    </#if>
    <#list error.codes as code>
        <#if args?? >
            <#assign errMsg=springMacroRequestContext.getMessage(code, args, nomessage) />
        <#else>
            <#assign errMsg=springMacroRequestContext.getMessage(code, nomessage) />
        </#if>
        <#if errMsg != nomessage>
            <#t>${errMsg?js_string}<#t><#break>
        </#if>
    </#list>
    <#if errMsg == nomessage>
        <#t>${error.defaultMessage?js_string}<#t>
    </#if>
</#macro>

<#--
 * formSingleSelectTextEnums
 *
 * Show a selectbox (dropdown) input element allowing a single value to be chosen
 * from a list of enumList.
 *  
 -->
<#macro formSingleSelectTextEnums path enumList attributes="" optional=false totalName='전체'>
    <@bind path/>
    <select id="${status.expression}" name="${status.expression}" ${attributes}>
    	<#if optional!'false' >
    		<option value="" >${(totalName)?html}</option>
        ...
        <#list enumList as enum>
            <option value="${enum}"<@checkSelected enum/>>${(enum.text)?html}</option>
        </#list>
    </select>
</#macro>

<#macro formSingleSelectGroupTextEnums path enumsInList groupName attributes="" optional=false totalName='전체'>
	<@bind path/>
	<select id="${status.expression}" name="${status.expression}" ${attributes}>
    	<#list enumsInList as enums>
    		<#if enums?size != 0>
    			<#list enums as enum>
    				<option class="${(groupName)?html}-${enums_index + 1}" value="${enum}" <@checkSelected enum />>${(enum.text)?html}</option>
    			</#list>
    		<#else>
	    		<#if optional!'false' >
	    			<option class="${(groupName)?html}-${enums_index + 1}" value="">${(totalName)?html}</option>
		    	</#if>
    		</#if>
    	</#list>
	</select>
...
<#macro formRadioButtonsTextEnums path enumList separator="" attributes="" >
    <@bind path/>
    <#list enumList as enum>
    <#assign id="${status.expression}${enum_index}">
    <input type="radio" id="${id}" name="${status.expression}" value="${enum.text?html}"<#if stringStatusValue == enum> checked="checked"</#if> ${attributes}<@closeTag/>
    <label for="${id}">${(enum.text)?html}</label>${separator}
    </#list>
</#macro>

<#-- 
	Deprecated.
	See nmp.ftl for zipcode
 -->
<#macro formInputZipCode1 path attributes="" fieldType="text">
    <@bind path/>
    <input type="${fieldType}" id="${status.expression}_1" name="${status.expression}_1" value="<#if stringStatusValue?length &gt; 3>${(stringStatusValue?substring(0,3))!''}</#if>" ${attributes}<@closeTag/>
</#macro>
<#macro formInputZipCode2 path attributes="" fieldType="text">
    <@bind path/>
    <input type="${fieldType}" id="${status.expression}_2" name="${status.expression}_2" value="<#if stringStatusValue?length &gt; 3>${(stringStatusValue?substring(3))!''}</#if>" ${attributes}<@closeTag/>
</#macro>
<#-- // Deprecated -->