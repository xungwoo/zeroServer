	...
	<@layout.put block="page-javascript-inline">
		...
		<script type="text/javascript">
			$("#shardIndex").change(function() {
				location.href = "${rc.contextPath}/post/admin/grid/" + this.value;
			});
			...
			var serverUrl = {
				gridUrl : "${rc.contextPath}/post/admin/grid/${shardIndex}",
				editUrl : "${rc.contextPath}/post/admin/grid/edit/${shardIndex}",
				excelUrl : "${rc.contextPath}/post/admin/excel/${shardIndex}",
				csvUrl : "${rc.contextPath}/post/admin/csv/${shardIndex}"
			}
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					mtype: "POST",
					datatype: "json",
					colNames:[' ', 'adminKey', 'Language', 'Text', '보상<br>분류', '보상수량<br>(장비코드)', '시작일시', '종료일시', '수정일시', '수정ID'],					
					colModel:[
						...
						{name:'postAdminKey', index:'postAdminKey', width:80, fixed:true, editable: true, hidden:true, key:true, sorttype:"integer", sortable:false},
						{name:'lang', index:'lang', width:80, fixed:true, sorttype:"string", editable: true, formatter:'select', edittype:"select", editoptions:{value:"ko:Korean;en:English"}, search:false, sortable:false},
						{name:'text', index:'text', width:290, fixed:true, sorttype:"string", editable: true, search:false, sortable:false},
