	...
	<@layout.put block="page-javascript-inline">
		...
		<script type="text/javascript">
			$("#shardIndex").change(function() {
				location.href = "${rc.contextPath}/log/unit/grid/" + this.value;
			});
			...
			var serverUrl = {
				unitLogGrid : "${rc.contextPath}/log/unit/grid/${shardIndex}",
				unitLogGridEdit : "${rc.contextPath}/log/unit/grid/edit/${shardIndex}",
				unitLogExcel : "${rc.contextPath}/log/unit/excel/${shardIndex}",
				unitLogCsv : "${rc.contextPath}/log/unit/csv/${shardIndex}"
			}
		
			var unitTypeJson = <#if unitTypeJson??>${unitTypeJson}<#else>{}</#if>;
			
			function pngFormatter(cellvalue, options, rowObject) {
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					mtype: "POST",
					datatype: "json",
					colNames:['Key', '유저ID', '유닛ID', '유닛타입', 'Image', '상태', '결과레벨', '저장일시'],					
					colModel:[
						...
						{name:'unitId', index:'unitId', width:300, fixed:true, sorttype:"integer", editable: false, searchoptions:{sopt: ['eq']} },
						{name:'unitType', index:'unitType', width:90, fixed:true, sorttype:"integer", editable: false, search:false, formatter: "select", edittype: "select", editoptions: {value: unitTypeJson} },
						{name:'unitImage', index:'unitImage', width:50, fixed:true, sorttype:"integer", editable: false, formatter: pngFormatter, search:false, sortable:false },
						{name:'status', index:'subCategory', width:60, fixed:true, sorttype:"integer", editable: false, search:false, formatter: "select", edittype: "select", editoptions: {value: "${UnitLogStatusOpts}"} },
						{name:'resultLevel', index:'grade', width:80, fixed:true, sorttype:"integer", editable: false, search:false },
