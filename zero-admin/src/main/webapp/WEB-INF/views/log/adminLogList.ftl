	...
	<@layout.put block="page-javascript-inline">
		...
		<script type="text/javascript">
			$("#shardIndex").change(function() {
				location.href = "${rc.contextPath}/log/admin/grid/" + this.value;
			});
			...
			var serverUrl = {
				unitLogGrid : "${rc.contextPath}/log/admin/grid/${shardIndex}",
				unitLogGridEdit : "${rc.contextPath}/log/admin/grid/edit/${shardIndex}",
				unitLogExcel : "${rc.contextPath}/log/admin/excel/${shardIndex}",
				unitLogCsv : "${rc.contextPath}/log/admin/csv/${shardIndex}"
			}
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					mtype: "POST",
					datatype: "json",	// `logKey`, `memo`, `type`, `modYmdt`, `modId`
					colNames:['Key', 'Data구분', 'Data Key', '변경사유', '수정일시', '수정ID'],					
					colModel:[
						{name:'logKey', index:'logKey', sorttype:"integer", editable: false, key: true, hidden:true},
						{name:'type', index:'type', width:100, fixed:true, sorttype:"integer", editable: false, formatter: "select", edittype: "select", editoptions: {value: "${AdminLogTypeOpts}"} },
						{name:'dataKey', index:'dataKey', width:300, sorttype:"integer", fixed:true, editable: false },
						{name:'memo', index:'memo', width:800, fixed:true, sorttype:"string", editable: false, search:false, sortable:false },
						{name:'modYmdt', index:'modYmdt', width:140, fixed:true, sorttype:"integer", editable: false, search:true, sortable:false },
						...
						searchfunc: function (pSearch) {
						    ...
						    $this.jqGrid("setGridParam", {postData: {
						        filters: {
						            groupOp: "AND",
						            rules: [
						                { field: "modYmdt", op: "le", "data": "" },
						                { field: "modYmdt", op: "ge", "data": "" }
						            ]
						        }
						    }});
