						...
						<div class="page-header">
							<h1>
								...
								<small>
									...
									Event BossRaid Meta 정보를 관리합니다.<br>
									<span style="color:red;"></span>
								</small>
							</h1>
						</div><!-- /.page-header -->
						
									<div class="alert alert-success" style="display:none;width:200px;height:100px;z-index:1;" id="loading">
										Processing...
									</div>						
						
									<div class="clearfix form-actions">
										<div class="col-md-9">
											<select id="bossRaidSelect" name="bossRaidSelect">
												<#list bossRaidMetaList as bossMeta>
													<option value="${bossMeta.bossRaidMetaId}">${bossMeta.titleKo}</option>
												</#list>
											</select>&nbsp;&nbsp; 										
											<button class="btn btn-info" type="button" id="ajaxStart" onclick="javascript:makeEventBoss();">
	...
	<@layout.put block="page-javascript-inline">
		...
		<script type="text/javascript">
			var serverUrl = {
				...
				function makeEventBoss() {
					var bossRaidMetaId = $('#bossRaidSelect').val();
					
					$.ajax({
						  url: "${rc.contextPath}${baseUrl}" + "/add-by-preset",
						  type: "POST",
						  dataType:"json",
						  context: document.body,
						  data: { bossRaidMetaId: bossRaidMetaId },
						  success: function(response){
						  		$("#loading").text('Add Preset Complete!!');
			...
			jQuery(function($) {
				jQuery(grid_selector).jqGrid({
					...
					mtype: "POST",
					datatype: "json",	
					colNames:[' ', 'key', '이벤트적용', 'bossRaidMetaId', 'title_ko', 'bossName_ko',  'title_en', 'bossName_en', 'goldReward', 'founderEquipmentType', 'gradeRatio','exposeStartYmdt', 'exposeEndYmdt', 
						'bossUnitType', 'bossUnitLevel', 'bossMaxHp', 'bossScale', 'bossDefensePhy', 'bossDefenseSpell', 'bossSkillLv0', 'bossSkillLv1', 'bossSkillLv2', 'bossImage', 'scene', 
						'texture0', 'texture1', 'texture2', 'minionUnitType', 'relativeMinionLevel', 'minionHpScale', 'minionScale', 'minionSkillLv0', 'minionSkillLv1', 'minionSkillLv2', 
						'minionSpawnNum', 'minionSpawnStart', 'minionSpawnInterval', 'fee', 'recommendationUnit', '수정일시', '수정ID' ],					
					colModel:[
						...
						{name:'isUse', index:'isUse', width:80, fixed:true, sorttype:"boolean", formatter:'select', editrules:{required:true}, editable: true, edittype:"select", editoptions:{value:"true:true;false:false"}, sortable:true, search:false},
						...
						{name:'goldReward', index:'goldReward', width:80, fixed:true, sorttype:"integer", editable: true, searchoptions:{sopt: ['eq']}, sortable:true},
						{name:'founderEquipmentType', index:'founderEquipmentType', width:80, fixed:true, sorttype:"integer", editable: true, searchoptions:{sopt: ['eq']}, sortable:true},
						{name:'gradeRatio', index:'gradeRatio', width:60, fixed:true, sorttype:"integer", editable: true, searchoptions:{sopt: ['eq']}, sortable:true},
						{name:'exposeStartYmdt', index:'exposeStartYmdt', width:150, fixed:true, sortable:false, editable: true, search:false },
						{name:'exposeEndYmdt', index:'exposeEndYmdt', width:150, fixed:true, sortable:false, editable: true, search:false },
						{name:'bossUnitType', index:'bossUnitType', width:80, fixed:true, sorttype:"integer", editable: true, searchoptions:{sopt: ['eq']}, sortable:true},
						{name:'bossUnitLevel', index:'bossUnitLevel', width:80, fixed:true, sorttype:"integer", editable: true, searchoptions:{sopt: ['eq']}, sortable:true},
						{name:'bossMaxHp', index:'bossMaxHp', width:80, fixed:true, sorttype:"integer", editable: true, searchoptions:{sopt: ['eq']}, sortable:true},
