...
@Slf4j
public abstract class MetaServiceImpl<M extends GenericMapper<T, K>, T, K> extends GenericServiceImpl<M, T, K> implements MetaService<T, K> {
	...
	@Cacheable(value="meta-cache", key="{#myMetaRevision, #root.targetClass, #id}")
	@Override
	@Transactional(propagation=Propagation.REQUIRES_NEW)
	@DataSourceAnnotation(DataSourceType.ZERO_META)
	...
	@Cacheable(value="meta-cache", key="{#myMetaRevision, #root.targetClass, #entity.hashCode()}")	
	@Override
	@Transactional(propagation=Propagation.REQUIRES_NEW)
	@DataSourceAnnotation(DataSourceType.ZERO_META)
	...
	@Cacheable(value="meta-cache", key="{#myMetaRevision, #root.targetClass}")
	@Override
	@Transactional(propagation=Propagation.REQUIRES_NEW)
	@DataSourceAnnotation(DataSourceType.ZERO_META)
