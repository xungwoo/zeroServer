...
import com.thirtygames.zero.common.mapper.admintool.AdmEquipMetaMapper;
...
@Slf4j
@Service("admEquipMetaService")
public class AdmEquipMetaServiceImpl extends AdminMetaServiceImpl<AdmEquipMetaMapper, EquipmentMeta, String> implements AdmEquipMetaService {
	...
	public Equipment generateEquip(Equipment equip, boolean isDecoRateWeight)  {
		...
		if (meta == null)
			throw new RestException("Not.found.EquipmentMeta!! params:: equipmentType::" + equip.getEquipmentType() + ":grade:" + equip.getGrade());
	...
	public int getMergedEquipmentType(int mergedGrade, int mergedClass) {
		return mapper.getRandomMergedEquipmentType(mergedGrade, mergedClass);
	}
	...
	public List<Equipment> generateGachaEquips(String accountId, ShopItem itemMeta)  {
		...
		for (long i = 0; i < eqCount; i++) {
			...
			equipTypeList = mapper.getNoGemEquipTypeList(grade, eqClass);
